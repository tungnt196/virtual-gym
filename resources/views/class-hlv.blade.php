...
<head>
    ...
    <script>
        ...
        var peer = new Peer('hlv', {host: '192.168.1.102', port: 9000, path: '/'});
...
//        }
        ...
        function connect(){
            ...
            navigator.mediaDevices.getUserMedia({video: true, audio : true})
                .then(function(stream) {
                    console.log('4');
                    var call = peer.call('hocVien', stream);
                    console.log('5');
                    setTimeout(function(){console.log(call)}, 3000);
                    call.on('stream', function(remoteStream) {
                        console.log('8');
                        // Show stream in some video/canvas element.
